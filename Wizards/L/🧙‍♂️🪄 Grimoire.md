---
wizard: L
wizard_name: "Wizard L"
wizard_motto: "Learn, Lead, and Launch with Purpose"
started_date:
favorite_topics: []
---

# 🧙‍♂️ Wizard L's Grimoire

> *"Learn, Lead, and Launch with Purpose"*
>
> Welcome back, Wizard L! Your journey to leadership starts here.
...
```dataviewjs
const studentWeeks = dv.pages('"Wizards/L"')
...
```dataviewjs
const currentWeek = dv.pages('"Wizards/L"')
  .where(p => p.file.path.includes("Week-") && p.file.name === "Tasks")
  .sort(p => p.week, 'desc')
  .limit(1);
...
```dataviewjs
const allWeeks = dv.pages('"Wizards/L"')
  .where(p => p.file.path.includes("Week-") && p.file.name === "Tasks")
  .sort(p => p.week, 'asc');
...
> *"Lead with vision, learn with humility. You've got this, Wizard L!"*
