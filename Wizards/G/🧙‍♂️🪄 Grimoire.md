...
> *"Growth through Consistent Practice"*
>
> Welcome back, Wizard G! Your DevOps journey continues here.
...
```dataviewjs
const studentWeeks = dv.pages('"Wizards/G"')
...
```dataviewjs
const currentWeek = dv.pages('"Wizards/G"')
  .where(p => p.file.path.includes("Week-") && p.file.name === "Tasks")
  .sort(p => p.week, 'desc')
  .limit(1);
...
```dataviewjs
const allWeeks = dv.pages('"Wizards/G"')
  .where(p => p.file.path.includes("Week-") && p.file.name === "Tasks")
  .sort(p => p.week, 'asc');
...
> *"Every master was once a beginner. Keep casting those spells, Wizard G!"*
